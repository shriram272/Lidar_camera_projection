        ...
        if self.initialize and self.latest_image is not None and self.latest_scan is not None:
            ...
            for i, scan_point in enumerate(self.latest_scan):
                ...
                if 0 <= image_x <= self.latest_image.shape[1] and 0 <= image_y <= self.latest_image.shape[0]:
                    ...
                    disparity = abs(image_x - P_camera[0, 0])  # Assuming P_camera[0, 0] is the horizontal position in the LiDAR frame
                    if disparity != 0:
                        focal_length = self.camera_intrinsic_matrix[0, 0]  # Assuming fx from the intrinsic matrix
                        depth = (baseline * focal_length) / disparity
                    else:
                        depth = float('inf')  # Infinite depth if no disparity

                    # Display depth on the image
                    cv2.putText(debug_image, f'{depth:.2f}m', (int(image_x), int(image_y) - 10),
